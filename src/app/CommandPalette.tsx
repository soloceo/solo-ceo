...
export function CommandPalette() {
  ...
  const quickCreate = (type: string) => {
    const tabMap: Record<string, string> = { task: "work", lead: "clients", transaction: "finance" };
    setActiveTab((tabMap[type] || "home") as any);
    setCommandPaletteOpen(false);
    setTimeout(() => window.dispatchEvent(new CustomEvent("quick-create", { detail: { type } })), 100);
  };

  const itemClass = "flex items-center gap-3 px-3 py-2 rounded-[var(--radius-6)] text-[13px] cursor-pointer transition-colors data-[selected=true]:bg-[var(--color-bg-tertiary)]";
            ...
            >
              ...
              >
                ...
                <Command.Input
                  ...
                  className="flex-1 bg-transparent text-[14px] outline-none placeholder:text-[var(--color-text-quaternary)]"
              ...
              <Command.List className="max-h-[360px] overflow-y-auto p-2" style={{ scrollbarWidth: "thin" }}>
                <Command.Empty className="py-6 text-center text-[13px]" style={{ color: "var(--color-text-tertiary)" }}>
                  ...
                  <Command.Group heading={<span className="section-label px-2 py-1">{t("app.searchResults" as any) || "Results"}</span>}>
                    {results.map((r) => (
                      <Command.Item
                        ...
                        style={{ color: "var(--color-text-primary)" }}
                      >
                        {r.type === "task" ? <Target size={15} style={{ color: "var(--color-text-tertiary)" }} /> : <Briefcase size={15} style={{ color: "var(--color-text-tertiary)" }} />}
                        <div className="flex-1 min-w-0">
                          <span className="truncate block">{r.title}</span>
                          {r.sub && <span className="text-[11px] truncate block" style={{ color: "var(--color-text-quaternary)" }}>{r.sub}</span>}
                        </div>

      ...
      <div className="flex flex-wrap gap-2 items-center mb-4">
        <div className="segment-switcher">
          {([["vertical", <LayoutGrid size={16} key="v" />], ["horizontal", <AlignJustify size={16} key="h" />]] as const).map(([m, icon]) => (
            <button key={m} onClick={() => setViewMode(m)} data-active={viewMode === m}>{icon}</button>
          ))}
        </div>
        <button onClick={() => setShowFunnel(f => !f)} className={`btn-ghost text-[13px] gap-1 ${showFunnel ? "ring-1" : ""}`} style={showFunnel ? { color: "var(--color-accent)", borderColor: "var(--color-accent)" } : undefined}>
            ...
            <div className="space-y-2">
              {LEAD_COLS.filter(col => col.id !== "lost").map((col, i) => {
                ...
                return (
                  <div key={col.id} className="flex items-center gap-3" style={{ paddingLeft: `${i * 6}%`, paddingRight: `${i * 6}%` }}>
                    <div className="flex-1 rounded-[var(--radius-4)] h-8 flex items-center justify-between px-3 text-[13px] transition-all" style={{ background: `color-mix(in srgb, ${col.color} 15%, transparent)`, color: col.color, maxWidth: `${maxW}%`, fontWeight: "var(--font-weight-medium)" as any }}>
                      <span>{col.title}</span>
              ...
              {(leads.lost?.length || 0) > 0 && (
                <div className="flex items-center gap-3 mt-1 pt-1 border-t" style={{ borderColor: "var(--color-border-primary)" }}>
                  <div className="flex-1 rounded-[var(--radius-4)] h-7 flex items-center justify-between px-3 text-[11px]" style={{ background: "color-mix(in srgb, var(--color-danger) 10%, transparent)", color: "var(--color-danger)", fontWeight: "var(--font-weight-medium)" as any }}>
            ...
            >
              <div className="flex items-center justify-between px-5 py-3 border-b" style={{ borderColor: "var(--color-border-primary)" }}>
                <div className="flex items-center gap-2.5">
                  <div className="flex h-7 w-7 items-center justify-center rounded-[var(--radius-4)]" style={{ background: "var(--color-accent-tint)", color: "var(--color-accent)" }}>{editId ? <Edit2 size={16} /> : <Plus size={16} />}</div>
                  <span className="text-[13px]" style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-semibold)" as any }}>{editId ? t("pipeline.panel.editLead" as any) : t("pipeline.panel.newLead" as any)}</span>
                </div>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t pb-safe" style={{ borderColor: "var(--color-border-primary)" }}>
                <div className="flex gap-2">
                  {editId && <button type="button" onClick={() => setDeleteId(editId)} className="btn-ghost text-[13px]" style={{ color: "var(--color-danger)" }}><Trash2 size={16} /> {t("common.delete" as any)}</button>}
                  {editId && <button type="button" onClick={() => { setConvertForm({ plan_tier: "", status: "Active", mrr: "", subscription_start_date: new Date().toISOString().split("T")[0] }); setShowConvert(true); }} className="btn-ghost text-[13px]" style={{ color: "var(--color-success)" }}><UserPlus size={16} /> {t("pipeline.convert.btn" as any)}</button>}
                </div>
                <div className="flex gap-2">
      ...
      {deleteId && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.4)" }}>
          <div className="card-elevated w-full max-w-sm p-5">
            ...
            <p className="text-[13px] mb-4" style={{ color: "var(--color-text-secondary)" }}>{t("pipeline.delete.warning" as any)}</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteId(null)} className="btn-secondary text-[13px]">{t("common.cancel" as any)}</button>
              <button onClick={() => deleteLead(deleteId)} className="text-[13px] px-4 py-2 rounded-[var(--radius-6)] text-white" style={{ background: "var(--color-danger)", fontWeight: "var(--font-weight-semibold)" as any }}>{t("pipeline.delete.confirm" as any)}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.4)" }}>
          <div className="card-elevated w-full max-w-md p-5 space-y-4">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button onClick={convertLead} disabled={converting} className="text-[13px] px-4 py-2 rounded-[var(--radius-6)] text-white flex items-center gap-1.5 disabled:opacity-50" style={{ background: "var(--color-success)", fontWeight: "var(--font-weight-semibold)" as any }}>
...
function LeadColumn({ col, items, onAdd, onEdit, onDelete, emptyText }: { col: { id: string; title: string; color: string }; items: any[]; onAdd: () => void; onEdit: (l: any) => void; onDelete: (id: number) => void; emptyText: string }) {
  return (
    <div className="flex flex-col min-w-[240px] flex-1 max-w-[360px] shrink-0 h-full">
      <div className="flex items-center justify-between mb-2 px-1">
        <div className="flex items-center gap-2">
          <div className="w-2.5 h-2.5 rounded-[var(--radius-2)]" style={{ background: col.color }} />
          <h3 className="text-[13px]" style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-semibold)" as any }}>{col.title}</h3>
          <span className="text-[11px] tabular-nums" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" as any }}>{items.length}</span>
        </div>
            ...
            <div className="flex-1 overflow-y-auto p-1.5 space-y-1 ios-scroll">
              {!items.length && (
                <div className="py-8 flex flex-col items-center gap-1.5">
                  ...
                  <span className="text-[11px]" style={{ color: "var(--color-text-quaternary)" }}>{emptyText}</span>
                </div>
              )}
    ...
    <div ref={provided.innerRef} {...provided.draggableProps}
      ...
      <div className="flex items-start justify-between gap-2 mb-1">
        ...
        <div className="min-w-0 flex-1">
          <h4 className="text-[13px] truncate" style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}>{lead.name}</h4>
          <p className="text-[11px] truncate mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{lead.industry}</p>
        </div>
        {lead.source && <span className="badge">{lead.source}</span>}
      </div>
      <p className="text-[11px] line-clamp-2 mb-1.5" style={{ color: "var(--color-text-secondary)" }}>{lead.needs}</p>
      <div className="flex items-center justify-between">
        <span className="badge"><Mail size={16} /> —</span>
        <button onClick={e => { e.stopPropagation(); onDelete(lead.id); }} className="p-0.5 rounded-[var(--radius-2)] opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "var(--color-text-secondary)" }}><Trash2 size={16} /></button>
      </div>
    </div>
  );
        ...
        return (
          <div key={col.id} className="card overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: "var(--color-border-primary)", borderTop: `2px solid ${col.color}` }}>
              <div className="flex items-center gap-2"><h3 className="text-[13px]" style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-semibold)" as any }}>{col.title}</h3><span className="text-[11px] tabular-nums" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" as any }}>{items.length}</span></div>
              <button onClick={() => onAdd(null, col.id)} className="btn-ghost p-0.5"><Plus size={16} /></button>
            </div>
            {!items.length ? <div className="px-4 py-4 flex items-center justify-center gap-2 text-[12px]" style={{ color: "var(--color-text-quaternary)" }}><UserPlus size={14} />{emptyText}</div> : (
              <div className="overflow-x-auto ios-scroll"><div className="flex gap-2 p-3 min-w-max">
                {items.map((lead: any) => (
                  <div key={lead.id} onClick={() => onEdit(lead, col.id)} className="w-[200px] shrink-0 cursor-pointer card-interactive p-3 group">
                    <h4 className="text-[13px] font-medium truncate mb-1" style={{ color: "var(--color-text-primary)" }}>{lead.name}</h4>
                    <p className="text-[11px] truncate mb-1" style={{ color: "var(--color-text-secondary)" }}>{lead.industry}</p>
                    <div className="flex items-center justify-between mt-2 pt-2 border-t" style={{ borderColor: "var(--color-border-primary)" }} onClick={e => e.stopPropagation()}>
                      <select value={col.id} onChange={e => onMove(lead.id, e.target.value)} className="appearance-none text-[11px] pl-2 pr-4 py-0.5 rounded-[var(--radius-4)] cursor-pointer input-base" style={{ fontWeight: "var(--font-weight-medium)" as any }}>{columns.map((c: any) => <option key={c.id} value={c.id}>{c.title}</option>)}</select>
                      <button onClick={() => onDelete(lead.id)} className="p-0.5 rounded-[var(--radius-2)] opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "var(--color-text-secondary)" }}><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))}
              </div></div>

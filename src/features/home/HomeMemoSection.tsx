...
/* ── Format date header for agenda grouping ── */
function formatDateHeader(dateStr: string, todayStr: string, lang: string): string {
...
export function HomeMemoSection() {
  ...
  /* Agenda: group by date when no day selected */
  const groupedMemos = useMemo(() => {
    if (selectedDay) {
      return [{ date: selectedDay, memos: memoTasks.filter(m => m.due && m.due.slice(0, 10) === selectedDay) }];
    }
    const groups: Record<string, Task[]> = {};
    const order: string[] = [];
    for (const m of memoTasks) {
      const key = m.due ? m.due.slice(0, 10) : "__none__";
      if (!groups[key]) { groups[key] = []; order.push(key); }
      groups[key].push(m);
    }
    return order.map(date => ({ date, memos: groups[date] }));
  ...
  const toggleTask = (task: Task) => {
    ...
    api.put(`/api/tasks/${task.id}`, { column: newColumn }).then(() => fetchTasks()).catch(() => { showToast?.(t("common.updateFailed")); });
  };

  const deleteTask = (id: number) => {
    api.del(`/api/tasks/${id}`).then(() => fetchTasks()).catch(() => { showToast?.(t("common.deleteFailed")); });
  };
  ...
  const saveEdit = () => {
    ...
    setEditingId(null);
    api.put(`/api/tasks/${editingId}`, { title: editTitle.trim(), due: buildDue(editDate, editTime) }).then(() => fetchTasks()).catch(() => { showToast?.(t("common.updateFailed")); });
  };
  ...
  const addMemo = () => {
    ...
    api.post("/api/tasks", { title, scope: memoScope, column: "todo", priority: "Medium", ...(due ? { due } : {}) }).then(() => fetchTasks()).catch(() => { showToast?.(t("common.saveFailed")); });
  };
  ...
  const monthTitle = lang === "zh"
    ...
    : new Date(viewYear, viewMonth, 1).toLocaleDateString("en-US", { year: "numeric", month: "long" });
    ...
    <section>
      <div className="card overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 pt-3 pb-1">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[14px] tabular-nums" style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-bold)" } as React.CSSProperties}>
              {monthTitle}
            </span>
            {!isCurrentMonth && (
              <button
                onClick={goToday}
                className="text-[11px] px-1.5 py-0.5 rounded-[var(--radius-4)] press-feedback"
                style={{ background: "var(--color-accent-tint)", color: "var(--color-accent)", fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}
              >
                {lang === "zh" ? "今天" : "Today"}
              </button>
            )}
          </div>
          <button onClick={() => goMonth(1)} className="btn-icon-sm press-feedback" aria-label="Next month">
        ...
        <div className="grid grid-cols-7 px-2 pb-2">
          {monthGrid.map((d, i) => {
            ...
            const isCurrentMon = d.getMonth() === viewMonth;
            ...
            return (
              <button
                ...
                className="flex flex-col items-center py-1 transition-all rounded-[var(--radius-6)]"
                style={{
                  opacity: isCurrentMon ? 1 : 0.3,
        ...
        {/* ── AI quick input ── */}
        <div
          className="flex items-center gap-2.5 mx-3 mb-2 px-3 py-2 rounded-[var(--radius-8)]"
        ...
        {/* ── Selected day chip ── */}
        {selectedDay && (
          <div className="flex items-center gap-2 mx-3 mb-2 px-2.5 py-1.5 rounded-[var(--radius-6)]" style={{ background: "var(--color-bg-tertiary)" }}>
            ...
            <span className="text-[12px] flex-1" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}>
              {formatDateHeader(selectedDay, todayDateStr, lang)}
            </span>
        ...
        <div className="px-3 pb-2">
          {groupedMemos.length > 0 && groupedMemos.some(g => g.memos.length > 0) ? (
            <div className="flex flex-col">
              {groupedMemos.map(({ date, memos }) => {
                if (memos.length === 0) return null;
                const dateHeaderLabel = date === "__none__"
                  ? (lang === "zh" ? "未定日期" : "No date")
                  : formatDateHeader(date, todayDateStr, lang);

                return (
                  <div key={date}>
                    {/* Date group header — only show when not filtering by single day */}
                    {!selectedDay && (
                      <div className="px-1 pt-3 pb-1">
                        <span className="text-[12px]" style={{ color: "var(--color-text-tertiary)", fontWeight: "var(--font-weight-bold)" } as React.CSSProperties}>
                          {dateHeaderLabel}
                        </span>
                      </div>
                    )}
            ...
            <div className="flex flex-col items-center py-8 gap-3">
              ...
              <div className="text-center">
                <div className="text-[14px]" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}>
                  {selectedDay
                    ? (lang === "zh" ? "这天没有备忘" : "No memos this day")
                    : t("home.memo.empty")}
                </div>

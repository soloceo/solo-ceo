    ...
    >
      {stats.map((s) => (
        <div key={s.label} className="flex-1 px-4 py-3 min-w-0">
          <div className="text-[11px] mb-0.5 truncate" style={{ color: "var(--color-text-tertiary)", fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}>
            {s.label}
          </div>
          <div className="flex items-baseline gap-1.5">
            <span
              className="text-[17px] tracking-tight tabular-nums"
              style={{ color: "var(--color-text-primary)", fontWeight: "var(--font-weight-bold)" } as React.CSSProperties}
            >
              {s.value}
            </span>
            {s.sub && (
              <span className="text-[11px] truncate" style={{ color: s.color, fontWeight: "var(--font-weight-medium)" } as React.CSSProperties}>
                {s.sub}
              </span>
            )}
          </div>

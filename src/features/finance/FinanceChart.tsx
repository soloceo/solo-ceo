      ...
      <div className="h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={chartData} margin={{ top: 5, right: 5, left: isMobile ? 0 : -15, bottom: 0 }}>
            ...
            <Bar dataKey="income" fill="var(--color-success)" radius={[3, 3, 0, 0]} opacity={0.8} isAnimationActive={!isMobile} />
            <Bar dataKey="expense" fill="var(--color-danger)" radius={[3, 3, 0, 0]} opacity={0.8} isAnimationActive={!isMobile} />
            <Line type="monotone" dataKey="net" stroke="var(--color-accent)" strokeWidth={2} dot={{ r: 3, fill: "var(--color-accent)" }} isAnimationActive={!isMobile} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>

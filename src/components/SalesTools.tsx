...
export default function SalesToolsPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 flex justify-end" style={{ zIndex: 700 }}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 shrink-0" style={{ borderBottom: "1px solid var(--color-border-primary)" }}>
          <h2 className="text-[15px]" style={{ fontWeight: "var(--font-weight-bold)" }}>{t("pipeline.salesTools" as any)}</h2>
          <button onClick={onClose} className="btn-icon" aria-label="Close sales tools"><X size={16} /></button>
        </div>

        {/* Tab bar — icon + text, horizontal pills */}
        <div className="flex gap-1 px-4 py-3 shrink-0" style={{ borderBottom: "1px solid var(--color-border-primary)" }}>
          {SEG_CONFIG.map(s => (
            <button
              key={s.id}
              onClick={() => setSeg(s.id)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-[var(--radius-12)] text-[15px] transition-all"
              style={seg === s.id
                ? { background: "var(--color-accent)", color: "var(--color-brand-text)", fontWeight: "var(--font-weight-medium)" }
                : { background: "var(--color-bg-tertiary)", color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" }
              }
            >
...
function CopyBtn({ text, showToast, t, size = 11 }: { text: string; showToast: (m: string) => void; t: (k: string) => string; size?: number }) {
  ...
  return <button onClick={copy} className="btn-icon-sm opacity-60 hover:opacity-100 transition-opacity" aria-label="Copy to clipboard"><Copy size={size} /></button>;
}
    ...
    return (
      <div className="space-y-1.5">
        <div className="flex items-center gap-1.5">
          <div className="flex-1 text-[15px] px-3 py-2 rounded-[var(--radius-12)]" style={{ background: "var(--color-bg-tertiary)", fontWeight: "var(--font-weight-medium)" }}>{zh}</div>
          <CopyBtn text={zh} showToast={showToast} t={t} size={16} />
        </div>
        <div className="flex items-center gap-1.5">
          <div className="flex-1 text-[15px] px-3 py-2 rounded-[var(--radius-12)]" style={{ background: "var(--color-bg-tertiary)", color: "var(--color-text-secondary)" }}>{en}</div>
  ...
  return (
    <div className="space-y-2">
      <div className="relative">
        <div className="flex items-center gap-1 mb-1"><span className="text-[13px] uppercase tracking-wider" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-bold)" }}>中文</span></div>
        <pre className="text-[13px] leading-relaxed px-3 py-3 rounded-[var(--radius-12)] whitespace-pre-wrap" style={{ background: "var(--color-bg-tertiary)", color: "var(--color-text-secondary)" }}>{zh}</pre>
        <div className="absolute top-6 right-1.5"><CopyBtn text={zh} showToast={showToast} t={t} /></div>
      </div>
      <div className="relative">
        <div className="flex items-center gap-1 mb-1"><span className="text-[13px] uppercase tracking-wider" style={{ color: "var(--color-text-secondary)", fontWeight: "var(--font-weight-bold)" }}>EN</span></div>
        <pre className="text-[13px] leading-relaxed px-3 py-3 rounded-[var(--radius-12)] whitespace-pre-wrap" style={{ background: "var(--color-bg-tertiary)", color: "var(--color-text-secondary)" }}>{en}</pre>
...
function EmailArsenal({ L, t, showToast }: { L: (o: { zh: string; en: string }) => string; t: (k: string) => string; showToast: (m: string) => void }) {
  ...
  return (
    <div className="py-3">
      <p className="px-5 text-[13px] mb-3" style={{ color: "var(--color-text-secondary)" }}>{t("pipeline.emailsHint" as any)}</p>
        ...
        return (
          <div key={tpl.id} style={{ borderBottom: "1px solid var(--color-border-primary)" }}>
            ...
            >
              ...
              <span className="flex-1 text-[15px]" style={{ fontWeight: "var(--font-weight-semibold)" }}>{L(tpl.label)}</span>
              <span className="text-[13px]" style={{ color: "var(--color-text-secondary)" }}>中/EN</span>
            </button>
            <AnimatePresence>
              {isOpen && (
...
function ScriptVault({ L, t, showToast }: { L: (o: { zh: string; en: string }) => string; t: (k: string) => string; showToast: (m: string) => void }) {
  ...
  return (
    <div className="py-3">
      <p className="px-5 text-[13px] mb-3" style={{ color: "var(--color-text-secondary)" }}>{t("pipeline.scriptsHint" as any)}</p>
      ...
      {SCRIPT_SCENARIOS.map((sc) => (
        <div key={sc.id} style={{ borderBottom: "1px solid var(--color-border-primary)" }}>
          ...
          >
            ...
            <span className="flex-1 text-[15px]" style={{ fontWeight: "var(--font-weight-medium)" }}>{L(sc.title)}</span>
            <span className="text-[13px]" style={{ color: "var(--color-text-secondary)" }}>中/EN</span>
          </button>
          <AnimatePresence>
            {open === sc.id && (
      ...
      <div className="px-5 pt-5 pb-8 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {PRICING_TIERS.map((tier) => (
            <div key={tier.id} className="card p-3 space-y-1" style={tier.tag ? { borderColor: "var(--color-accent)", borderWidth: 2 } : {}}>
              <div className="flex items-center justify-between">
                <span className="text-[15px]" style={{ fontWeight: "var(--font-weight-bold)" }}>{L(tier.name)}</span>
                {tier.tag && <span className="text-[13px]" style={{ color: "var(--color-accent)", fontWeight: "var(--font-weight-bold)" }}>{L(tier.tag)}</span>}
              </div>
              <div className="text-[16px]" style={{ color: "var(--color-accent)", fontWeight: "var(--font-weight-bold)" }}>{tier.price}</div>
              <ul className="space-y-0.5">
                {tier.features.slice(0, 3).map((f, i) => (
                  <li key={i} className="text-[13px]" style={{ color: "var(--color-text-secondary)" }}>✓ {L(f)}</li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="flex gap-2 overflow-x-auto ios-scroll">
          {PRICING_STAGES.map((s) => (
            <div key={s.id} className="card p-3 min-w-[140px] shrink-0 space-y-1">
              ...
              <div className="text-[13px]" style={{ color: "var(--color-text-secondary)" }}>{L(s.period)}</div>
              <div className="text-[15px]" style={{ color: "var(--color-success)", fontWeight: "var(--font-weight-bold)" }}>{L(s.target)}</div>
            </div>
          ))}
        </div>
      </div>

...
export default function DailyProtocol() {
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <div>
            ...
            <p className="text-[13px] mt-0.5" style={{ color: "var(--color-text-secondary)" }}>{t("home.protocol.desc" as any)}</p>
          </div>
          {streak.count > 1 && (
            <span className="flex items-center gap-1 text-[13px]" style={{ color: "var(--color-warning)", fontWeight: "var(--font-weight-bold)" }}>
              <Flame size={16} /> {streak.count}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[13px] tabular-nums" style={{ color: allDone ? "var(--color-success)" : "var(--color-text-secondary)", fontWeight: "var(--font-weight-medium)" }}>
            {done}/5
          </span>
          {done > 0 && (
      ...
      {/* progress bar */}
      <div className="w-full h-1.5 rounded-full overflow-hidden mb-3" style={{ background: "var(--color-bg-tertiary)" }}>
        <div
          className="h-full rounded-full transition-all duration-300"
          style={{ width: `${pct}%`, background: allDone ? "var(--color-success)" : "var(--color-accent)" }}
        />
      </div>

      {/* celebration */}
      {justCompleted && (
        <div className="mb-3 py-3 px-4 rounded-[var(--radius-16)] text-center text-[15px] celebrate-bounce" style={{ background: "var(--color-success-light)", color: "var(--color-success)", fontWeight: "var(--font-weight-semibold)" }}>
        ...
        {PROTOCOL_STEPS.map((step, idx) => {
          ...
          return (
            <button
              ...
              style={checked ? { background: "color-mix(in srgb, var(--color-success) 5%, transparent)" } : {}}
            >
              {checked ? (
                <CheckSquare size={16} className="shrink-0" style={{ color: "var(--color-success)" }} />
              ) : (
                <Square size={16} className="shrink-0" style={{ color: "var(--color-border-secondary)" }} />
              )}
              <div className="flex-1 min-w-0 flex items-center gap-2">
                <span className="badge shrink-0" style={{ color: "var(--color-accent)", fontSize: 10 }}>{L(step.time)}</span>
                <span className={`text-[15px] truncate ${checked ? "line-through" : ""}`} style={checked ? { color: "var(--color-text-secondary)" } : {}}>
                  {L(step.title)}
                </span>
              </div>
            </button>

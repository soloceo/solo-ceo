...
import { Key, Save, Check, ExternalLink, Play, Loader2, AlertCircle, Upload, Trash2, Moon, Sun, Globe, LogOut, Cloud, CloudOff, Lock, Mail, Plus, Edit2, Package } from 'lucide-react';
...
/* ── Plan Manager ──────────────────────────────────────────────── */
function PlanManager({ showToast }: { showToast: (msg: string) => void }) {
  const { t } = useT();
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={openNew} className="btn-ghost text-[11px] flex items-center gap-1" style={{ color: "var(--accent)" }}><Plus size={16} /> {t("common.add" as any)}</button>
      </div>
      <div className="card divide-y" style={{ borderColor: "var(--border)" }}>
        {plans.length === 0 && <div className="p-4 text-center text-[13px]" style={{ color: "var(--text-secondary)" }}>{t("settings.noPlans" as any)}</div>}
        {plans.map(p => (
          <div key={p.id} className="flex items-center justify-between px-4 py-3">
            <div>
              <div className="text-[13px] font-medium" style={{ color: "var(--text)" }}>{p.name}</div>
              <div className="text-[11px]" style={{ color: "var(--text-secondary)" }}>${Number(p.price || 0).toLocaleString()}/mo {p.deliverySpeed ? `· ${p.deliverySpeed}` : ""}</div>
            </div>
            <div className="flex gap-1">
              <button onClick={() => openEdit(p)} className="btn-ghost p-1.5"><Edit2 size={16} /></button>
              <button onClick={() => del(p.id)} className="btn-ghost p-1.5" style={{ color: "var(--danger)" }}><Trash2 size={16} /></button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="mobile-page max-w-2xl lg:max-w-3xl mx-auto px-4 py-4 md:px-8 md:py-6 lg:py-8 relative">
      ...
      <div className="space-y-8">
        ...
        {/* ── Plan Manager ── */}
        <PlanManager showToast={showToast} />
